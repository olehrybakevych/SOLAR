...
$(function(){
	...
	$('.info-tabs__item').on('click', function(e) {
		e.preventDefault(); 
		// для того чтобы ссылки перестали быть ссылками

	$('.info-tabs__item').removeClass('info-tabs__item--active');
	// читать нужно как - при клике на .info-tabs__item у всех убрать класс info-tabs__item--active,
	 // если используется removeClass ОБЯЗАТЕЛЬНО убрать точку

	$(this).addClass('info-tabs__item--active');
	// чтобы определить ту на которую кликнул, используем конструкцию $(this)
	// чтобы добавить туда класс используем .addClass('')
	// на ту на которую кликнул, нужно чтобы добавился этот клас info-tabs__item--active

	$('.info-tabs__content').removeClass('info-tabs__content--active');
	// у всех info-tabs__content нужно убрать info-tabs__content--active
	// все исчезло

	$($(this).attr('href')).addClass('info-tabs__content--active');
	// attr - атрибут
	// чтобы когда кликаешь на другую кнопку и появился контент нужно..
	// при клике на другую кнопку вытащи attr - (атрибут) href
	// добавляем активный класс, который мы удалили, чтобы отобразить инфо
	// теперь вместо $($(this).attr('href')), подставится то что я нажал (#tab1, #tab2)
	});
...
// когда подключили файлы jQuery
// обращаемся к родительскому элементу слайдера (тот в котором находяться элементы слайдера)
// добавить .slick() чтобы заработал слайдер
// fade - плавное переключение
